...
<template>
  <div class="pt-8">
    <ul class="space-y-8" >
        <li v-for="item in props.list" class="flex flex-col text-center">
          <h3 class="font text-sm md:text-2xl" >{{ item.label }}<UIcon v-if="item.isVegetarian" class="text-green-500 mb-1 md:mb-2" name="i-ph-leaf-bold"/><UIcon v-if="item.spicelevel ?? 0 > 0" v-for="n in item.spicelevel ?? 0" name="i-ph-fire-bold" class="text-red-500 mb-1 md:mb-2" /></h3>
