  ...
  <div class="rounded-md bg-slate-200 dark:bg-slate-700 bg-opacity-95 max-w-2xl mx-8 md:mx-auto shadow-xl">
    <TabGroup :selectedIndex="Number(route.query.item)" v-if="isMenu">
      <TabList class="grid grid-flow-col">
        <Tab as="template" v-slot="{ selected }"v-for="tab in types" class="first:rounded-tl-md last:rounded-tr-md"><button class="h-12" :class=" { 'bg-slate-500 text-white shadow-2xl border-2 border-slate-600':selected, 'bg-slate-600 text-slate-100 hover:bg-slate-500': !selected } " @click="replaceQuery(tab.index)">{{ tab.label }}</button></Tab>
      </TabList>
      <div class="border-x-slate-700 dark:border-x-slate-600 border-x-8 border-b-slate-700 dark:border-b-slate-600 border-b-8 rounded-b-md">
